...
const IcoFlask    = () => (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 3h6"/><path d="M10 3v7l-3.5 7a2 2 0 0 0 1.8 2.9h7.4a2 2 0 0 0 1.8-2.9L14 10V3"/></svg>)
const IcoBarChart = () => (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>)
const IcoBox      = () => (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>)
...
const IcoTruck    = () => (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" rx="1"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>)
const IcoWarn     = () => (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>)
...
export function Inventario() {
  ...
  const TABS: { key: Tab; label: string; icon: React.ReactNode; badge?: number }[] = [
    { key: 'insumos', label: 'Insumos', icon: <IcoFlask />, badge: inv.insumos.length },
    { key: 'stock', label: 'Stock', icon: <IcoBarChart />, badge: inv.alertasStock.length > 0 ? inv.alertasStock.length : undefined },
    { key: 'lotes', label: 'Lotes', icon: <IcoBox />, badge: inv.lotesVencidos.length + inv.lotesPorVencer.length > 0 ? inv.lotesVencidos.length + inv.lotesPorVencer.length : undefined },
    { key: 'mermas', label: 'Mermas', icon: <IcoTrash /> },
    { key: 'transferencias', label: 'Transferencias', icon: <IcoTruck /> },
  ]
          ...
          <div>
            ...
            {inv.alertasStock.length > 0 && (
              <p className="text-sa-coffee text-sm mt-1">
                <span className="inline-flex items-center gap-1"><IcoWarn /> {inv.alertasStock.length} insumo{inv.alertasStock.length > 1 ? 's' : ''} con stock bajo mínimo</span>
              </p>
            )}
          </div>
          {CTA[tab] && (
