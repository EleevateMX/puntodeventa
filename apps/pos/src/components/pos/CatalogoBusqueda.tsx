    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-2 px-4 py-3 overflow-x-auto flex-shrink-0">
        ...
        {categorias.map((cat) => (
          ...
          >
            {cat.cocinas && (
              <span>{cat.cocinas.slug === 'alimentos' ? '🍽️' : '🥤'}</span>
            )}
            {cat.nombre}
          </button>
        ))}
              ...
              >
                {p.imagen_url ? (
                  <img
                    src={p.imagen_url}
                    alt={p.nombre}
                    className="w-16 h-16 rounded-sa object-cover mb-2"
                  />
                ) : (
                ...
                <p className="font-mono text-sm font-medium text-sa-strawberry mt-2">
                  ${p.precio}
                </p>
              </button>
            ))}
